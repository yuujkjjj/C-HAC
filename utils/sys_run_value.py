...
class PolicyRunner:
    ...
    def run_an_episode(
        ...
        while not (done or info["TimeLimit.truncated"]):
            ...
            obs_t = torch.from_numpy(np.expand_dims(obs, axis=0).astype("float32"))
            # print(info['action'], info['raw_action'])
            raw_action = info["raw_action"]
            action_t = torch.from_numpy(np.expand_dims(raw_action, axis=0).astype("float32"))
            StochaQ = controller.q1(obs_t, action_t)
            mean_1, std_1 = StochaQ[..., 0], StochaQ[..., -1]

            StochaQ = controller.bq1(obs_t, action_t)
            mean_1, std_1 = StochaQ[..., 0], StochaQ[..., -1]

            action_random = np.random.uniform(
                env.action_space.low, env.action_space.high, env.action_space.shape
            )
            # action_random=[0,0]
            obs_t = torch.from_numpy(np.expand_dims(obs, axis=0).astype("float32"))
            action_t = torch.from_numpy(np.expand_dims(action_random, axis=0).astype("float32"))
            StochaQ = controller.bq1(obs_t, action_t)
            mean_2, std_2 = StochaQ[..., 0], StochaQ[..., -1]
            # print(mean_1, std_1, mean_2, std_2)
            mean_diff=(mean_1-mean_2).detach().item()
            std=std_1.detach().item()
            # print((mean_1-mean_2).detach().item(), std_1.detach().item(),std_2.detach().item())
            # print(scipy.stats.norm.cdf(mean_diff/std))
    ...
    def compute_action(self, obs: np.ndarray, networks: Any) -> np.ndarray:
        ...
        StochaQ = networks.bq1(batch_obs, action)
        mean, std = StochaQ[..., 0], StochaQ[..., -1]
        action = action.detach()[0].numpy()

        logits = networks.policy_rl(batch_obs)
        action_distribution = networks.create_action_distributions(logits, True)
        action_rl, logp_rl = action_distribution.sample()
        action_rl = action_distribution.mode()
        StochaQ_rl = networks.bq1(batch_obs, action_rl)
        mean_rl, std_rl = StochaQ_rl[..., 0], StochaQ_rl[..., -1]
        action_rl = action_rl.detach()[0].numpy()
        logp_rl = logp_rl.detach()[0].numpy()
        print(mean.detach(),mean_rl.detach(),(mean-mean_rl).detach())
        if mean_rl > mean:
            use_rl = True
            action = action_rl
            logp = logp_rl
